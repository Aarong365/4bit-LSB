...
def embed_image_in_image(cover_image_path, secret_image_path, output_image_path):
    ...
    将秘密图像嵌入到封面图像中，采用论文中4bit-LSB隐写方法：
      ...
      3. 构造头信息（4字节，记录打包后数据长度），并与打包数据拼接后转换为二进制字符串。
      4. 检查封面图像嵌入容量（cover_width * cover_height * 3 * 4 bit）；若不足则报错。
      5. 逐像素将封面图像每个像素3通道的低4位替换为数据的4位。
    """
    ...
    # 头信息：4字节，记录打包后数据长度
    header = len(packed_secret).to_bytes(4, byteorder='big')
...
def extract_image_from_image(stego_image_path, output_secret_image_path):
    ...
    从隐写图像中提取出秘密图像。
      ...
      2. 前32位为头信息（4字节），表示打包后秘密数据长度。
      3. 根据头信息提取对应比特串，并转换为字节。
      4. 解包得到秘密图像每个像素每通道的4位数据，再乘以16还原到近似8位范围，
         重构秘密图像后保存。
    """
    ...
    header_bits = bit_string[:32]  # 4字节头信息
    packed_length = int(header_bits, 2)
    total_packed_bits = packed_length * 8
    packed_bits = bit_string[32:32 + total_packed_bits]
    ...
    # 解包得到4位数据数组
    num_elements = (packed_length * 2)  # 每个字节含2个4位数据
    secret_4bit = unpack_4bit_bytes(packed_data, num_elements)
    # 还原到8位：乘以16
    secret_8bit = (secret_4bit.astype(np.uint16) * 16).astype(np.uint8)
    # 根据秘密图像尺寸重构图像
    # 为恢复秘密图像尺寸，可从原始秘密图复制（这里假设秘密图尺寸与out_secret对应）
    secret_img_ref = Image.open(output_secret_image_path).convert("RGB")
    secret_width, secret_height = secret_img_ref.size
    recovered_array = secret_8bit.reshape((secret_height, secret_width, 3))
...
def compute_bpp(cover_image_path, secret_image_path):
    ...
    根据封面图像尺寸和秘密图像数据大小计算比特每像素 (bpp)：
      bpp = ((4 + packed_secret_length) * 8) / (width * height)
    其中 4 字节为头信息，packed_secret_length为打包后秘密图像数据的字节数。
    """
    cover_img = Image.open(cover_image_path).convert("RGB")
    ...
    packed_secret = pack_4bit_array(secret_4bit)
    total_bits = (4 + len(packed_secret)) * 8
