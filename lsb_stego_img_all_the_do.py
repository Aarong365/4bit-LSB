...
#############################################
# 4bit-LSB 图像隐写实现部分（不存储头信息，假定封面图与秘密图尺寸相同）
#############################################
...
def extract_image_from_image(stego_image_path, output_secret_image_path):
    ...
    从隐写图像中提取出秘密图像。
      ...
      2. 假定秘密图像尺寸与封面图像相同，故提取的总4位数据数 = width * height * 3。
...
if __name__ == '__main__':
    input_cover_folder = r"E:\hxr\paper_codefile\datasets\MSCOCO-5k"
    input_secret_folder = r"E:\hxr\paper_codefile\datasets\MSCOCO-5k"
    output_root = "image_lsb_stego_img_coco5k_256"
